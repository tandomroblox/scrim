...
async def enable_channel(message):
	...
	if message.channel.id not in bot.queue_channels.keys():
		...
		await message.channel.send(embed=error_embed("The bot is already enabled on this channel."))
...
async def add_player(message, queue_name=None):
	qc = bot.queue_channels.get(message.channel.id)
	if not qc:
		await message.channel.send(embed=error_embed("The bot is not enabled in this channel."))
		return

	try:
		await qc.add_member(message.author, queue_name)
	except Exception as e:
		await message.channel.send(embed=error_embed(str(e)))


async def remove_player(message, queue_name=None):
	qc = bot.queue_channels.get(message.channel.id)
	if not qc:
		await message.channel.send(embed=error_embed("The bot is not enabled in this channel."))
		return
	...
	try:
		await qc.remove_members(message.author)
	except Exception as e:
		await message.channel.send(embed=error_embed(str(e)))
...
async def on_message(message):
	...
	queue_name = parts[1] if len(parts) > 1 else None
	...
	if command == "enable":
		...
		await add_player(message, queue_name)

	elif command == "remove":
		await remove_player(message, queue_name)
